...
original_input = inputFile;
%%
fromPntList = string(inputFile.A);
toPntList = string(inputFile.B);

allPointsInstances = sort([fromPntList;toPntList]);

uniquePoints = unique(allPointsInstances);

[tf,idx] = ismember(allPointsInstances,uniquePoints);


instances = hist(idx,length(uniquePoints))';


%%
instancesT = table(instances,uniquePoints);
