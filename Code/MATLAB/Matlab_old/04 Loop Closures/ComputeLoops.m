...
% creating edges table that will be used for the graph creation
...
% creating nodes table that will be used for the graph creation
...
% returns a graph without multiple edges or self-loops
% When several edges occur between the same two nodes, only the first edge (as defined in G.Edges) is kept. Edge properties are preserved.
G = simplify(G);
...
temp.Edges.dHO(isnan(temp.Edges.dHO))=temp.Edges.dH(isnan(temp.Edges.dHO));
loopSummary.misc_mm_w_grav(i) = round(sum(temp.Edges.dHO.*temp.Edges.Direction)*1000,2);
