function [resultsCell] = calculateLoopCombinations(levellingData)
    ...
    %
    % Input:
    %   levellingData - An N-by-8 matrix where each row represents a
    ...
    if size(levellingData, 2) ~= 8
        error('Input levellingData must have 8 columns.');
    end
    ...
    for i = 1:numCombinations
        ...
        for j = 1:numSegments
            ...
            % The corrected height difference for this line's measured direction
            corrected_dH = lineData(5) + lineData(8);
            
            if lineData(2) == path_p1 
                % Measurement direction matches the path traversal direction
                misclosure = misclosure + lineData(5);
                misclosureWithGravity = misclosureWithGravity + lineData(8);
            else 
