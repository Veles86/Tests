...
def summarize_loops_data(loops_input):
    ...
    # Check if the field 'AbsMeasHeightDiffMM' exists, if not, add it
    fields = [field.name for field in arcpy.ListFields(loops_input)]
    if 'AbsMeasHeightDiffMM' not in fields:
        AddField(loops_input, 'AbsMeasHeightDiffMM', 'DOUBLE')
    if 'AbsCorrHeightDiffMM' not in fields:
        AddField(loops_input, 'AbsCorrHeightDiffMM', 'DOUBLE')
    if 'MaxRankByMeasHeightDiff' not in fields:
        AddField(loops_input, 'MaxRankByMeasHeightDiff', 'SHORT')
    if 'MaxRankByCorrHeightDiff' not in fields:
        AddField(loops_input, 'MaxRankByCorrHeightDiff', 'SHORT')
